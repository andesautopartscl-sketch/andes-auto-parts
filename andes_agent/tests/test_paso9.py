...
SUPPLIERS = [
    ...
    {
        "id": 2,
        "nombre": "JAVIER TONKIN",
        "empresa": "RC REPUESTO CENTER",
        "rut": "79.656.210-2",
        "giro": "COMERCIO",
        "comuna": "MAIPU",
        "ciudad": "SANTIAGO",
    ...
    def do_POST(self):
        ...
        if path == "/internal/agent/v1/ventas/suppliers":
            ...
            if supplier_id is not None:
                rows = [row for row in rows if row["id"] == int(supplier_id)]
                if not rows:
                    self._reply(404, {"ok": False, "error_code": "not_found", "message": "Proveedor no encontrado"})
                    return
            elif rut:
                rows = [
                    row
                ...
                rows = [
                    row
                    for row in rows
                    if q in str(row["nombre"]).upper()
                    or q in str(row.get("empresa") or "").upper()
                    or q in str(row["giro"]).upper()
                ]
...
class GetSupplierPaso9Tests(unittest.TestCase):
    def setUp(self):
    ...
    def test_01_por_nombre(self):
        code, body = self._invoke("get_supplier", {"q": "ALBERT"})
        ...
        self.assertEqual(body["data"]["items"][0]["empresa"], "ANDES AUTO PARTS LTDA")
        self.assertEqual(_FakeERP.last.get("path"), "/internal/agent/v1/ventas/suppliers")
        self.assertIsNone(_FakeERP.last.get("cookie"))

    def test_02_por_empresa(self):
        code, body = self._invoke("get_supplier", {"q": "RC REPUESTO"})
        self.assertEqual(code, 200)
        self.assertEqual(body["data"]["items"][0]["id"], 2)

    def test_03_por_rut(self):
        code, body = self._invoke("get_supplier", {"rut": "78.074.288-7"})
        self.assertEqual(code, 200)
        self.assertEqual(body["data"]["items"][0]["id"], 1)
        self.assertEqual(_FakeERP.last["body"].get("rut"), "78.074.288-7")

    def test_04_por_id(self):
        code, body = self._invoke("get_supplier", {"id": 2})
        self.assertEqual(code, 200)
        self.assertEqual(body["data"]["items"][0]["empresa"], "RC REPUESTO CENTER")

    def test_05_inexistente_id(self):
        code, body = self._invoke("get_supplier", {"id": 99999})
        self.assertEqual(code, 404)
        self.assertEqual(body["error_code"], "not_found")

    def test_06_inexistente_rut(self):
        code, body = self._invoke("get_supplier", {"rut": "11.111.111-1"})
        self.assertEqual(code, 404)
        self.assertEqual(body["error_code"], "not_found")

    def test_07_q_vacio_resultados(self):
        code, body = self._invoke("get_supplier", {"q": "EMPTY-PROV"})
        self.assertEqual(code, 200)
        self.assertEqual(body["data"]["items"], [])
        self.assertEqual(body["data"]["count"], 0)

    def test_08_sin_filtro(self):
        code, body = self._invoke("get_supplier", {})
        self.assertEqual(code, 400)
        self.assertEqual(body["error_code"], "invalid_args")

    def test_09_limit(self):
        code, body = self._invoke("get_supplier", {"q": "ALBERT", "limit": 20})
        self.assertEqual(code, 200)
        self.assertEqual(body["meta"]["limit"], 20)
        self.assertFalse(body["meta"]["truncated"])

    def test_10_limit_mayor_20(self):
        code, body = self._invoke("get_supplier", {"q": "ALBERT", "limit": 21})
        self.assertEqual(code, 400)
        self.assertEqual(body["error_code"], "invalid_args")

    def test_11_campo_adicional(self):
        code, body = self._invoke("get_supplier", {"q": "ALBERT", "order_by": "nombre"})
        self.assertEqual(code, 400)
        self.assertEqual(body["error_code"], "invalid_args")

    def test_12_sql(self):
        code, body = self._invoke("get_supplier", {"q": "SELECT * FROM proveedores"})
        self.assertEqual(code, 400)
        self.assertEqual(body["error_code"], "invalid_args")
        code, body = self._invoke("get_supplier", {"q": "ALBERT", "sql": "DROP TABLE"})
        self.assertEqual(code, 400)
    ...
    def test_14_cookie(self):
        code, body = self._invoke("get_supplier", {"q": "ALBERT"}, headers=self._auth({"Cookie": "session=abc"}))
        self.assertEqual(code, 401)
    ...
    def test_schema_limit(self):
        with self.assertRaises(SchemaError):
            validate_tool_arguments("get_supplier", {"q": "x", "limit": 21})
