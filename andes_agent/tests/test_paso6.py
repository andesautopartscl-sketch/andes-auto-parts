...
INGRESO_ITEMS = [
    {
        ...
        "cantidad": 2,
        "costo_neto": 4500.0,
        "precio_venta_neto": 8900.0,
        "margen_pct": 35.0,
        "proveedor_rut": "76.111.111-1",
...
def _public_ingreso(row, include_finance=True):
    item = {
        "fecha": row["fecha"],
    ...
    if include_finance:
        for key in ("costo_neto", "precio_venta_neto", "margen_pct"):
            if key in row:
                item[key] = row[key]
    return item
...
class _FakeERP(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/internal/agent/v1/bodega/ingresos":
            ...
            if codigo == "EMPTY-ING":
            ...
            elif codigo == "MANY-ING":
                rows = [
                    {
                        "fecha": "2026-07-31",
                        "numero_documento": str(idx),
                        "proveedor": "PROV",
                        "anulado": False,
                        "codigo": "MANY-ING",
                        "descripcion": "X",
                ...
                rows = list(INGRESO_ITEMS)
            proveedor = str(body.get("proveedor") or "").strip().upper()
            numero = str(body.get("numero_documento") or "").strip()
            ...
            if numero:
                rows = [row for row in rows if numero in str(row["numero_documento"])]
            if fecha_desde:
            ...
            items = [_public_ingreso(row, include_finance=include_finance) for row in rows[:limit]]
            payload = {
                "ok": True,
                "tool": "get_ingresos",
            ...
            if codigo:
                ...
                payload["data"]["descripcion"] = "FILTRO DIESEL"
...
class GetIngresosPaso6Tests(unittest.TestCase):
    def setUp(self):
    ...
    def test_01_ingreso_existente(self):
        code, body = self._invoke("get_ingresos", {"numero_documento": "302"})
        ...
        self.assertFalse(body["data"]["items"][0]["anulado"])
    ...
    def test_09_limit(self):
        code, body = self._invoke("get_ingresos", {"codigo": "2404", "limit": 20})
        self.assertEqual(code, 200)
        self.assertEqual(body["meta"]["limit"], 20)
        self.assertFalse(body["meta"]["truncated"])

    def test_10_limit_mayor_20(self):
        code, body = self._invoke("get_ingresos", {"codigo": "2404", "limit": 21})
        self.assertEqual(code, 400)
        self.assertEqual(body["error_code"], "invalid_args")

    def test_11_campo_adicional(self):
        code, body = self._invoke("get_ingresos", {"codigo": "2404", "order_by": "fecha"})
        self.assertEqual(code, 400)
        self.assertEqual(body["error_code"], "invalid_args")

    def test_12_sql_en_argumentos(self):
        code, body = self._invoke("get_ingresos", {"codigo": "SELECT * FROM ingresos"})
        self.assertEqual(code, 400)
        self.assertEqual(body["error_code"], "invalid_args")
        code, body = self._invoke("get_ingresos", {"codigo": "2404", "sql": "DROP TABLE"})
        self.assertEqual(code, 400)
        self.assertEqual(body["error_code"], "invalid_args")
    ...
    def test_14_cookie(self):
        code, body = self._invoke("get_ingresos", {"codigo": "2404"}, headers=self._auth({"Cookie": "session=abc"}))
        self.assertEqual(code, 401)
    ...
    def test_17_redaccion_sin_permiso_financiero(self):
        ...
        for field in ("costo_neto", "precio_venta_neto", "margen_pct", "proveedor_rut", "email", "token"):
            self.assertNotIn(f'"{field}"', blob)

    def test_18_audit_sin_secretos(self):
        self._invoke("get_ingresos", {"codigo": "2404"})
        raw = self.audit_path.read_text(encoding="utf-8")
        self.assertIn("get_ingresos", raw)
        self.assertNotIn(TOKEN, raw)
        self.assertNotIn("Authorization", raw)
        self.assertNotIn("Bearer ", raw)
        record = json.loads(raw.strip().splitlines()[-1])
        self.assertTrue(record["success"])
        self.assertEqual(record["tool_name"], "get_ingresos")
        self.assertEqual(record["arguments_redacted"]["codigo"], "2404")

    def test_19_ingreso_anulado(self):
        code, body = self._invoke("get_ingresos", {"codigo": "ANULADO-ING"})
        self.assertEqual(code, 200)
        self.assertTrue(body["data"]["items"][0]["anulado"])

    def test_20_truncated(self):
        code, body = self._invoke("get_ingresos", {"codigo": "MANY-ING", "limit": 20})
        self.assertEqual(code, 200)
        self.assertEqual(body["data"]["count"], 20)
        self.assertTrue(body["meta"]["truncated"])
        code, body = self._invoke("get_ingresos", {"codigo": "EXACT-20", "limit": 20})
        self.assertEqual(code, 200)
        self.assertEqual(body["data"]["count"], 20)
        self.assertFalse(body["meta"]["truncated"])

    def test_21_erp_apagado(self):
        down = create_app(
            Settings(
                environment="local",
                service_token=TOKEN,
    ...
    def test_schema_rechaza_limit_alto(self):
        with self.assertRaises(SchemaError):
            validate_tool_arguments("get_ingresos", {"codigo": "2404", "limit": 21})
