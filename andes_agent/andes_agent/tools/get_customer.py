"""get_customer — READ tool. Cliente directory (ventas_clientes only).

Never merges proveedores or oc_clientes. PII and finance fields are copied only
if the ERP included them.
"""
...
def _public_customer(row: Any) -> dict[str, Any] | None:
    ...
    if "cliente_mayorista" in row:
        item["cliente_mayorista"] = bool(row.get("cliente_mayorista"))
    margen = _as_float(row.get("margen_descuento_pct"))
    if margen is not None and "margen_descuento_pct" in row:
        item["margen_descuento_pct"] = margen
    return item
...
def handle(arguments: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        payload = adapter.call("ventas/customers", payload_args, actor_user=actor, method="POST")
    except AdapterError as exc:
        message = exc.message or "Customer lookup failed"
